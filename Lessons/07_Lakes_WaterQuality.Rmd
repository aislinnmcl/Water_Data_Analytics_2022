...
# if we don't see a 1:1 relationship between P and chlorophyll, could be an example of something else limiting chlorophyll like N
...
LAGOSMN <-
  ...
  filter(state_name == "Minnesota" & year(sampledate) >= 1990 & month(sampledate) >= 6 & month(sampledate) <= 9) %>%
  filter(hu4 == "0401" | hu4 == "0701" | hu4 == "0702" | hu4 == "0703" | hu4 == "0704" | hu4 == "0902")

ggplot(LAGOSMN, aes(x = chla, y = tp)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) +
  facet_wrap(vars(hu4)) +
  scale_y_log10() +
...
model <- lm(data = LAGOSMN, log10(chla) ~ log10(tp) * hu4)
summary(model)
```
...
LAGOScovariates <- LAGOSjoined %>%
  ...
  select(chla, tn, tp, doc, secchi, #dissolved organic 
...
> Positive correlation between lake area + percent forest, slope + percent forest (ppl less likely to build houses/develop steeper sloped areas), max depth + lake area. Negative correlation is 

> Putting multiple things in a model at once can be an issue if there is a lot of covariance between predictor variables. Might want to use a scheme to filter down predictor variables into subset. Simple approaches are AIC (step function that builds model up from 0 predictor variables to optimal amount, or down), some parametric linear regressions (e.g. lasso), machine learning (e.g. random forest)