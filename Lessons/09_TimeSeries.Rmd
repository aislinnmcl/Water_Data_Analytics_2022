...
model <- lm(data=NeuseFlow, Discharge ~ Date)
summary(model)
...
> Seasonality, climate change, any events like effluent release or dam removal that may have caused the big spikes in discharge. Linear trend analysis is not particularly useful for this dataset
...
> Focusing on something that is not changing as quickly (e.g. soil chemistry). Might also do linear trend as a rough first pass / exploratory technique. Could be more useful for annual data rather than daily data.
...
plot(Neuse_Decomposed) # bars on righthand side show difference in scale on different plot facets
...
ggplot(Neuse_Components) +
  ...
  labs(x = "", y = "Discharge (cfs)")
# trend in plot above is always positive, but we might expect it to be negative as discharge trends up and down
...
NeuseCond_processed <- NeuseCond %>%
  ...
  mutate(Year = year(Date),
         Month = month(Date),
         Date_monthrounded = floor_date(Date, "month")) %>% # rounds date down to nearest month
  arrange(Date)
...
```{r}
#1
cond.ts <- ts(NeuseCond_monthly[[3]], frequency = 12)
#2
cond.decomp <- stl(cond.ts, s.window = "periodic")
#3
plot(cond.decomp)

cond.components <- as.data.frame(cond.decomp$time.series[,1:3])
cond.components <- mutate(cond.components,
                      Observed = NeuseCond_monthly$Conductivity,     
                      Date = NeuseCond_monthly$Date)

ggplot(cond.components) +
  geom_line(aes(y = Observed, x = Date), color = "gray", size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
  geom_hline(yintercept = 0, lty = 2) +
