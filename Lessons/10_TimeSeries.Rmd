...
> aggregation: taking a certain time step and taking data and taking central tendency, sum, more spaced out time step. good reason to aggregate is some ts analyses require that there is no temporal autocorrelation. e.g. you have daily data and want to get monthly data. you add up your daily data to get monthly data

> subsampling: a lot of approaches require you to subsample data so you can build a model with part of your data and then test on the rest of your data. e.g. you have daily data and want to get monthly data. you pick the daily data on the first day of each month to get monthly data.
...
```{r}
Condplot <-
ggplot(subset(NeuseCond_monthly, !is.na(Month)), aes(x = as.factor(Month), y = Conductivity)) +
  geom_boxplot(outlier.shape = NA) + # have to remove outliers to use geom_jitter()
...
ggplot(NeuseFlow_monthly, aes(x = as.factor(Month), y = Discharge_acftmo)) +
  geom_boxplot() +
  labs(x = "Month", y = "Discharge (ac*ft/mo)")

plot_grid(Condplot, Flowplot, ncol = 1, align = "v")
```
...
# Inspect results
NeuseCond_trend # positive z score tells you spacing betweeen points is in positive direction over time, p-value tells you how likely that outcome was. the larger your z score (-ve or +ve), the lower your pvalue will be. S calculates sum of differences over time.
summary(NeuseCond_trend) #asterisks show significance
...
> We have seen a monotonic increase in discharge over time (positive z score) but not that strong. Wouldn't directly compare s scores between discharge and conductivity but can compare z scores. Z score is just asking "is there a trend" S score is telling you how strong the trend is and the directionality. Seasonality is comparing within months (so each January compared to each January). positive trend in discharge over time is restricted to late summer months.
...
#p value 0.01 is < .05 and is significant so data are stationary over time
...
acf(NeuseFlow_timeseries) # autocorrelation decays over time, and we're seeing some interesting seasonality with acf plot going from positive to negative
pacf(NeuseFlow_timeseries) # decays a lot faster and not much seasonality
