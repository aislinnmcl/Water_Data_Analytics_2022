...
*nutrient pollution - nitrogen/phosphorus, causes eutrophication / algal blooms
*sediment loading - covers up habitat
*thermal pollution - affects wildlife
*other contaminants - heavy metals, pesticides
...
# if columns have same data but are name different things, can do:
# LAGOSlocations <- left_join(LAGOSlocus, LAGOSstate, by = c("state_zoneid = colname2"))
...
> Lakes seem to have lower visibility in summer (secchi depth is lower), which makes sense because we have more chlorophyll
...
# dots below line where TSI.chl is higher than TSI.secchi is when reduction in clarity is mostly from phytoplankton/algae
# dots above line where TSI.secchi is higher than TSI.chl is when there is more turbidity so reduction in clarity is not from phytoplankton. problematic because you're assigning something as eutrophic when there isn't much primary production associated with it
...
ggplot(LAGOStrophic, aes(x = TSI.chl, y = TSI.tp, color = samplemonth)) +
  ...
  scale_color_viridis_c(option = "magma", end = 0.8, direction = -1) +
  theme(legend.position = "top")
# dots above line show situation where something other than phosphorus is limiting primary production, maybe nitrogen
# dots below line show any increase in total phosphorus is very tightly correlated with increase in chlorophyll
...
ggplot(LAGOStrophic, aes(x = state, y = TSI.chl)) +
  ...
  #geom_rect(aes(xmin = 0, ymin = 0, xmax = 18, ymax = 40))
# can use geom_rect if you want to fill in tiers eutrophic mesotrophic etc.
