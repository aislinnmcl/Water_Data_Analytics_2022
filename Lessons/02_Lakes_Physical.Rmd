...
# install.packages("tidyverse") - can't knit rmd file if install.packages is uncommented
...
NTLdata$sampledate <- as.Date(NTLdata$sampledate, format = "%m/%d/%y") # provide format as it exists before change
class(NTLdata$sampledate)
...
NTLdata_PeterPaul <- NTLdata %>%
  filter(lakename %in% c("Paul Lake", "Peter Lake")) # %in% is like "includes"
...
# QA the one data point at 20 m depth (aka remove the point over 20m). 
Pauldata <- Pauldata %>%
  filter(depth < 20)
...
ggplot(subset(Pauldata, year4 == 1993), 
  ...
  scale_y_reverse() +
  scale_color_viridis_c() +
  labs(x = expression("Dissolved Oxygen (mg/L)"), y = "Depth (m)", color = "DOY") 
...
Thermocline deepens as year goes on. Most variability is at the beginning of the year.
...
ggplot(Pauldata_thermo, aes(x = daynum, y = thermocline, color = year4)) +
  geom_point() +
  scale_color_viridis_c() +
  labs(x = "Day", y = "Thermocline Depth (m)") + 
