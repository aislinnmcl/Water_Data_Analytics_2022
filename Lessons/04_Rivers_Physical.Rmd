...
library(lubridate)
library(lfstat) # low-flow statistics
...
EnoDischarge <- EnoDischarge %>%
  mutate(Year = year(Date), 
         WaterYear = water_year(Date, origin = "usgs")) %>% #water_year fxn from lfstat pkg
  filter(WaterYear != "1963") #only have 30 days of data for this year, so we're removing it

# Water Year is a factor. We want to re-classify as numeric.
EnoDischarge$WaterYear <- as.numeric(as.character(EnoDischarge$WaterYear)) #have to tell R it's a character first

EnoRecurrence <- 
  EnoDischarge %>%
  filter(WaterYear < 1994) %>% #picked 30 years randomly, just for example
  group_by(WaterYear) %>%
...
```{r}
# can calc recurrence intervals on as little as 10 yrs of data
# stationarity - need to use more years of data, but if we're seeing rapid changes in hydrology, using historical data is not that helpful
ggplot(EnoRecurrence, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() +
  scale_x_log10() +
  geom_smooth(method = "lm") # color = "black", se = FALSE - makes it look a little nicer

Eno.RImodel <- lm(data = EnoRecurrence, PeakDischarge ~ log10(RecurrenceInterval))
summary(Eno.RImodel) #R2 - over 90% of our variability is explained by this recurrence interval
...
EnoRecurrence.Late <- 
  EnoDischarge %>%
  filter(WaterYear >= 1992) %>% #choosing the most recent 30 years
  group_by(WaterYear) %>%
  summarise(PeakDischarge = max(Discharge)) %>% 
  mutate(Rank = rank(-PeakDischarge), 
         RecurrenceInterval = (length(WaterYear) + 1)/Rank, 
...
# urbanization + channelization of flow leads to high peaks that disappear faster, less urban areas have shorter, broader peaks
