...
maine <- LAGOScombined %>%
  filter(state == "ME") %>%
  group_by(lagoslakeid, nhd_lat, nhd_long, lake_area_ha, maxdepth) %>%
  summarise(secchi.mean = mean(secchi)) %>%
  drop_na()

maine_sf <- st_as_sf(maine, coords = c("nhd_long", "nhd_lat"), crs = 4326)
...
ggplot() +
  geom_sf(data = filter(states, ID == "maine")) +
  geom_sf(data = maine_sf, aes(color = secchi.mean, size = lake_area_ha))
...
plot <- ggplot() +
  geom_sf(data = filter(states, ID == "maine")) +
  geom_sf(data = maine_sf, aes(size = maxdepth))
plot

plot2 <- ggplot() +
  geom_sf(data = filter(states, ID == "maine")) +
  geom_sf(data = maine_sf, aes(color = secchi.mean))
plot2

plot3 <- ggplot() +
  geom_sf(data = filter(states, ID == "maine")) +
  geom_sf(data = maine_sf, aes(size = maxdepth, color = secchi.mean))
plot3

legend <- get_legend(plot)
legend2 <- get_legend(plot2)

object <- plot_grid(plot3 + theme(legend.position = "none"), legend, legend2, ncol = 3)
