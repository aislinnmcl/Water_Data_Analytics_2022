...
*if soil is already hydrated, there will be more runoff
*vegetation near stream can absorb water
*storm magnitude
*topography of watershed (area and slope)
*drainage area and number of tributaries
...
# order in which you load packages dictates dominant fxns
library(EcoHydRology) #load this one first bc it has a lot of same fxns as tidyverse
library(tidyverse)
...
Baseflow is dependent on groundwater, so it makes sense that it fluctuates especially in times of year when the water table is low.
...
ggplot(EnoDischarge, aes(x = Date, y = Discharge)) +
  ...
  scale_y_log10() + # using log10 scale for y axis is helpful to compare very small numbers to very large numbers
  labs(x = "Year", y = "Discharge (cfs)")
...
ggplot(EnoDischarge, aes(x = Baseflow_lfstat, y = bt)) +
  ...
  geom_abline(slope = 1, intercept = 0, color = "darkred") + #1 to 1 line, compares methods
  geom_smooth(method = lm)
# How would we test that statistically?

# linear model?
model <- lm(data = EnoDischarge, bt ~ Baseflow_lfstat)
summary(model)

# could also use paired t-test
...
EnoDischarge_summary <- EnoDischarge %>%
  group_by(WaterYear) %>%
  summarise(Discharge.acft.yr = sum(Discharge)*723.968, # converting from cfs to acrefeet per year
 ...
 mutate_if(is.numeric, round, 2) # limits number of decimal places - if col is numeric, round 2 places


ggplot(EnoDischarge_summary, aes(x = WaterYear, y = Discharge.acft.yr)) +
  geom_line() +
  geom_line(aes(y = Baseflow.acft.yr), lty = 2) # edit line type
...
** Example illustrates groundwater depletion at Coolidge site that affect flows at Dodge City
...
CoolidgeDischarge <- readNWISdv(siteNumbers = "07137500", 
                             parameterCd = "00060",
                             startDate = "1960-10-01",
                             endDate = "2005-09-30")

names(CoolidgeDischarge)[4:5] <- c("Discharge", "Approval.Code")


CoolidgeDischarge <- CoolidgeDischarge %>%
  mutate(baseflow.lfstat = baseflow(Discharge),
         stormflow.lfstat = Discharge - baseflow.lfstat,
         year = year(Date),
         water.year = water_year(Date))

CoolidgeDischarge$water.year <- as.numeric(as.character(CoolidgeDischarge$water.year))

plot.CoolidgeBaseflow <-
  ggplot(CoolidgeDischarge, aes(x = Date, y = Discharge)) +
  geom_line(color = "gray") +
  geom_line(aes(x = Date, y = baseflow.lfstat))
  # scale_y_log10()
plot.CoolidgeBaseflow

```

Some legend color stuff
```{r}

ggplot(BitterrootDischarge, aes(x = Date)) +
  geom_line(aes(y = Discharge, color = "total discharge")) +
