...
NCdata.filtered <-
  NCdata %>%
  select(OrganizationIdentifier, OrganizationFormalName, ActivityStartDate, ActivityConductingOrganizationText, MonitoringLocationIdentifier, ActivityDepthHeightMeasure.MeasureValue, ResultMeasureValue, CharacteristicName) %>%
  group_by(OrganizationIdentifier, OrganizationFormalName, ActivityStartDate, ActivityConductingOrganizationText, MonitoringLocationIdentifier, ActivityDepthHeightMeasure.MeasureValue, CharacteristicName) %>%
  summarise(ResultMeasureValue.mean = mean(ResultMeasureValue)) %>%
  pivot_wider(names_from = CharacteristicName, values_from = ResultMeasureValue.mean) %>%
  mutate(Month = month(ActivityStartDate)) %>%
  filter(Month %in% c(5:10) & ActivityDepthHeightMeasure.MeasureValue <= 1)
